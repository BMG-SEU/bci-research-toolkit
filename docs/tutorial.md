...
| 源定位分析 | `ao run workflows/source-localization.yaml -i file_path=@data.fif` | .fif | 方案+代码 |
| 功能连接 | `ao run workflows/connectivity-analysis.yaml -i file_path=@data.fif` | 数据+频段 | 方案+代码 |
| 论文摘要 | `ao run workflows/paper-abstract.yaml -i background="..." -i method="..."` | 4个输入 | 摘要.md |
| 图表生成 | `ao run workflows/figure-generation.yaml -i data_desc="..."` | 数据描述 | Python代码 |
| 数据读取 | `ao run workflows/data-analysis-bridge.yaml -i file_path=@data.fif` | .fif文件 | 元数据+方案 |
